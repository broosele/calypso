from ..src.gui import MPLProfilePlot
...
algorithm = zh_l16c(gf_low=0.35, gf_high=0.85)
...
dive = DivePlan.from_table(start_gas_supply_set=start_gas_supply_set, table=tec40_4).dive.resample(Time(10))


# Extracting values
deco = algorithm.compartiment_profiles(depth_profile=dive.depth_profile, gas_usage_profile=dive.gas_usage_profile, gas_supply_set=start_gas_supply_set)


ts = [time.value/60 for time in dive.timeline]
ds = [-dive.depth_profile[t].value for t in dive.timeline]
ps = {gas_supply_name: [dive.gas_supply_profile[time].gas_supplies[gas_supply_name].pressure.value/1e5 for time in dive.timeline] for gas_supply_name in start_gas_supply_set.gas_supplies}
...
aps = [pressure_from_depth(dive.depth_profile[t]).value for t in dive.timeline]
...
for time, gas_supply_set in dive.gas_supply_profile.gas_supply_sets.items():
    print(f"{time}: {gas_supply_set}")

MPLProfilePlot(dive, deco).show()
